...
```{r}
#Split data to two sets that classify IPA and Ales
IPABeers = beersAndBrew %>% select(Beer_Name, ABV, IBU, Style) %>% filter(str_detect(Style, "IPA"))
AleBeers = beersAndBrew %>% select(Beer_Name, ABV, IBU, Style) %>% filter(str_detect(Style, "Ale"))
#Create new column for type
IPABeers$Type = "IPA"
AleBeers$Type = "Ale"

#Combine into one for KNN classification
...
knn(beersALEIPA[,2:3], beerTest, beersALEIPA$Type, k = 7, prob = TRUE)
...
## Part 9: Supplemental and Bonus information
